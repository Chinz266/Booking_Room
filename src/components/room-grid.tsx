...
export function RoomGrid() {
  ...
  return <div className="availability-view">
    ...
    <div className="room-grid">{rooms.map((room)=>{
      ...
      const statusClass=loading?"loading":busy.length||unavailable?"busy":"free";
      return <article className={`room-card ${!loading&&(busy.length||unavailable)?"has-bookings":"is-free"}`} key={room.id}>
        <div className="room-card-top"><span className="room-icon">{String(room.room_number).slice(0,1)}F</span><span className={`availability-badge ${statusClass}`}>{loading?"กำลังตรวจสอบ":unavailable?"ปิดใช้งาน":busy.length?`ไม่ว่าง ${busy.length} ช่วง`:"ว่างทั้งวัน"}</span></div>
        <strong className="room-number">{room.room_name}</strong><span className="room-floor">อาคารเรียน · ชั้น {String(room.room_number).slice(0,1)}</span>
        <div className="busy-times">{loading?<div className="time-skeleton"><span/><span/></div>:unavailable?<p>ห้องนี้ยังไม่เปิดรับการจอง</p>:busy.length?<><span>เวลาที่ไม่ว่าง</span>{busy.slice(0,3).map((item)=><p key={item.id}><b>{item.start_time}–{item.end_time}</b>&nbsp;น.</p>)}{busy.length>3&&<small>และอีก {busy.length-3} ช่วงเวลา</small>}</>:<p className="all-day-free"><b>เลือกเวลาได้ตามต้องการ</b><span>ยังไม่มีรายการจองในวันนี้</span></p>}</div>
        <Link className={`button room-button ${unavailable||loading?"disabled":""}`} aria-disabled={unavailable||loading} href={unavailable||loading?"#":`/booking?room=${room.room_number}&date=${date}`}>{loading?"กำลังตรวจสอบเวลา":unavailable?"ยังไม่เปิดให้จอง":`จองห้อง ${room.room_number}`}</Link>
      </article>})}</div>
  </div>;
}
