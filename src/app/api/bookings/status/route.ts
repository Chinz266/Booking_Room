...
export async function POST(request:Request) {
  try {
    ...
    const result = await appsScriptPost({action:actions[status],adminKey:process.env.APPS_SCRIPT_ADMIN_KEY,data:{id,reason:String(reason||"").trim(),actor:(await currentUser())?.username||"admin"}});
