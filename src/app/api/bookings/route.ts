...
export async function GET(request:Request) {
  try {
    ...
    const user=scope==="admin"||scope==="mine"?await currentUser():null;
...
export async function POST(request:Request) {
  try {
    ...
    const [user,bookingSupported]=await Promise.all([currentUser(),cachedAppsScriptGet("getCapabilities",{},60000).then(result=>result.data?.bookingProfile===1).catch(()=>false)]);if(!user)return Response.json({success:false,message:"กรุณาเข้าสู่ระบบก่อนจอง"},{status:401});
    ...
    if(!bookingSupported)return Response.json({success:false,message:"ผู้ดูแลต้องอัปเดต Apps Script เวอร์ชันใหม่ก่อน เพื่อบันทึกประเภทผู้จองและสาขาวิชา"},{status:503});
